...
export class PurchaseOrderValidators extends BaseValidator {
  ...
  static createPurchaseOrder = z.object({
    body: z.object({
      ...
      status: z.enum(["PENDING", "APPROVED", "ORDERED", "RECEIVED", "CANCELLED"]).optional().default("PENDING"),
      totalAmount: z.number().positive("Total amount must be positive").optional(),
      expectedDate: z.string().datetime("Invalid expected date format").optional().nullable(),
      notes: z.string().optional().nullable(),
      items: z
        .array(
          z.object({
            productVariantId: z.string().uuid("Invalid Product Variant ID format"),
            orderedQuantity: z.number().int().positive("Ordered quantity must be greater than 0"),
  ...
  static receivePurchaseOrder = z.object({
    body: z.object({
      receivedDate: z.string().datetime("Invalid received date format").optional().nullable(),
      notes: z.string().optional().nullable(),
    }),
  });
