...
export class PurchaseOrderController extends BaseController {
  ...
  getAllPurchaseOrders = this.asyncHandler(
    async (req: Request, res: Response) => {
      ...
      return this.successResponse(res, result.data || result, 200, {
        message: "All purchase order get successfully.",
  ...
  getPurchaseOrderById = this.asyncHandler(
    async (req: Request, res: Response) => {
  ...
  createPurchaseOrder = this.asyncHandler(
    async (req: Request, res: Response) => {
  ...
  updatePurchaseOrder = this.asyncHandler(
    async (req: Request, res: Response) => {
      const id = req.validatedBody?.params?.id || req.params.id;
      const data = req.validatedBody?.body || req.body;
  ...
  receivePurchaseOrder = this.asyncHandler(
    async (req: Request, res: Response) => {
      const id = req.validatedBody?.params?.id || req.params.id;
      const data = req.validatedBody?.body || req.body;
  ...
  // Adjust already received purchase order items and stock
  adjustReceivedPurchaseOrder = this.asyncHandler(
    async (req: Request, res: Response) => {
      const id = req.validatedBody?.params?.id || req.params.id;
      const data = req.validatedBody?.body || req.body;
      const result = await this.service.updateReceivedPurchaseOrder(id, data);
      return this.successResponse(res, result, 200, {
        message: "Purchase order adjusted successfully",
      });
    },
  );

  softDeletePurchaseOrder = this.asyncHandler(
    async (req: Request, res: Response) => {
      const id = req.validatedBody?.params?.id || req.params.id;
  ...
  restorePurchaseOrder = this.asyncHandler(
    async (req: Request, res: Response) => {
      const id = req.validatedBody?.params?.id || req.params.id;
  ...
  hardDeletePurchaseOrder = this.asyncHandler(
    async (req: Request, res: Response) => {
      const id = req.validatedBody?.params?.id || req.params.id;
